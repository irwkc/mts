...
IMAGE_GEN_RE = re.compile(
    ...
    # Создай + что-то визуальное
    r"создай\s+(?:мне\s+)?(?:картинк\w*|изображен\w*|фото|фотографию|рисунок|арт\b|постер|логотип|баннер|иконк\w*)|"
    # Покажи/хочу картинку
    r"покажи\s+(?:мне\s+)?(?:картинку|фото|изображение|рисунок)|"
    r"хочу\s+(?:картинку|фото|изображение|рисунок)|"
    # Сгенерируй / разг. «сгенери», «сгенерь» (без кода/текста)
    ...
    # Фото/картинка как существительное с контекстом
    r"(?:сделай|создай|нарисуй|придумай|генерируй)\s+(?:мне\s+)?фото\w*|"
    # Английские варианты
    r"text-to-image|generate\s+an?\s+image|draw\s+(?:me\s+)?|image\s+generation|flux|sdxl"
    r")",
    re.I,
)
