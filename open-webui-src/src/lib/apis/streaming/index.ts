...
type TextStreamUpdate = {
	...
	error?: any;
	usage?: ResponseUsage;
	/** gpthub/gena structured presentation events (choices[0].delta.gena) */
	gena?: unknown;
};
...
async function* openAIStreamToIterator(
	...
	while (true) {
		...
		try {
			...
			const textVal = delta?.content ?? '';
			const genaVal = delta?.gena;
			yield {
				done: false,
				value: textVal,
				...(genaVal !== undefined ? { gena: genaVal } : {})
			};
		} catch (e) {
...
async function* streamLargeDeltasAsRandomChunks(
	...
	for await (const textStreamUpdate of iterator) {
		...
		let genaOnce = textStreamUpdate.gena;
		...
		if (content.length < 5) {
			yield {
				...
				value: content,
				...(genaOnce !== undefined ? { gena: genaOnce } : {})
			};
			genaOnce = undefined;
			continue;
		}
