...
<div
	class="gena-style-picker mt-3 mb-1 w-full max-w-2xl rounded-2xl border border-gray-200/80 dark:border-indigo-500/25 bg-gradient-to-br from-gray-50/95 via-white/90 to-teal-50/30 dark:from-gray-900/90 dark:via-indigo-950/40 dark:to-gray-950/95 p-3 shadow-sm relative z-20"
	role="region"
	aria-label="Выбор стиля презентации"
>
	<p class="text-xs font-medium text-gray-500 dark:text-gray-400 mb-2.5 uppercase tracking-wide">
		Стиль презентации
	</p>
	<div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
		{#each styles as s (s.id)}
			<button
				...
				class="gena-style-btn text-left rounded-xl px-3 py-2.5 text-sm transition-all border
					{picked === s.id
			...
			>
				<span class="font-semibold block leading-tight">{s.label}</span>
				{#if s.hint}
					<span class="text-[0.7rem] text-gray-500 dark:text-gray-400 mt-0.5 block leading-snug"
						>{s.hint}</span
					>
				{/if}
			</button>
	...
	{#if picked}
		<p class="text-[0.7rem] text-teal-600 dark:text-teal-400/90 mt-2.5">Запускаю сборку…</p>
	{/if}
</div>
